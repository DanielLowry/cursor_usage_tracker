 ...
 */
export function parseCapturedPayload(item: { url?: string; payload: Buffer; kind: 'html' | 'network_json' }): unknown {
  if (item.kind === 'network_json') {
    try {
      return JSON.parse(item.payload.toString('utf8'));
    } catch (err) {
      console.warn('runScrape: network_json parse failed', err);
      return null;
    }
  }
...
export async function ensureBlob(
  ...
  return { id: blob.id, created: true };
}
...
async function persistCaptured(captured: Array<{ url?: string; payload: Buffer; kind: 'html' | 'network_json' }>, keepN: number) {
  ...
  for (const item of captured) {
    ...
    const parsedPayload = parseCapturedPayload(item);

    const { id: blobId, created } = await ensureBlob(item, contentHash, now);
    if (created) saved += 1;

    if (!parsedPayload) continue;

    try {
      const normalizedEvents = normalizeCapturedPayload(parsedPayload, now, blobId ?? null);
