// Relative path: apps/worker/src/workers/scraper/index.ts
...
import { parseCapturedItem, toNormalizedEvents } from './normalize';
...
export async function ensureBlob(
  ...
  return { id: blob.id as string, created: true };
}
...
async function persistCaptured(captured: Array<{ url?: string; payload: Buffer; kind: 'html' | 'network_json' }>, keepN: number) {
  ...
  for (const item of captured) {
    ...
    const payload = parseCapturedItem(item);
    ...
    if (!payload) {
      console.warn('runScrape: unable to parse captured payload', { kind: item.kind, url: item.url });
      continue;
    }

    try {
      const normalizedEvents = toNormalizedEvents(payload, now, blob.id);
      const { tableHash, billingPeriodStart, billingPeriodEnd, totalRowsCount } = buildStableViewHash(normalizedEvents);
      const maxExisting = await findMaxExistingCapture(billingPeriodStart, billingPeriodEnd);
